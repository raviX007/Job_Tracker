"""Pydantic models for YAML profile config validation.

Validates ~100 fields at startup. If validation fails, prints the exact
field and error, then exits. Never lets the system start with invalid config.
"""
...
class JobDict(TypedDict, total=False):
    """Type hint for job dicts flowing through the pipeline.

    All scrapers return list[JobDict]. Using TypedDict (not dataclass)
    so existing dict access patterns continue working unchanged.
    """
    title: str
    company: str
    location: str
    source: str              # "indeed", "remoteok", "greenhouse", etc.
    discovered_via: str      # "jobspy", "remoteok", "jsearch", etc.
    description: str
    job_url: str
...
class ProfileConfig(BaseModel):
    """Top-level config model. Validates the entire YAML profile."""
